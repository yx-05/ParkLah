...
import { delay, DELAY_TIME_1, forwardConnectionError, isNotConnectionError, isRedisInstance, randomUUID, } from '../utils';
...
export class Worker extends QueueBase {
    ...
    async getNextJob(token, { block = true } = {}) {
        ...
        while (nextJob) {
            ...
            await this.handleFailed(new UnrecoverableError(unrecoverableErrorMessage), nextJob, token, () => false);
            ...
            if (!this.closing && bclient.status === 'end') {
                try {
                    ...
                    if (bclient.status === 'end' &&
                        isNotConnectionError(reconnectError)) {
