(function (chai, stream, module, fs) {
	...
	function readStringJS(length) {
		...
		while (position$1 < end) {
			...
			if ((byte1 & 0x80) === 0) {
				...
				// 2 bytes
				if (byte1 < 0xc2 || position$1 >= end || (src[position$1] & 0xc0) !== 0x80) {
					units.push(0xFFFD);
				} else {
				...
				if (position$1 >= end || (byte2 & 0xc0) !== 0x80 ||
					(byte1 === 0xe0 && byte2 < 0xa0) || (byte1 === 0xed && byte2 >= 0xa0)) {
					units.push(0xFFFD);
				} else {
					position$1++;
					if (position$1 >= end || (src[position$1] & 0xc0) !== 0x80) {
						units.push(0xFFFD);
					} else {
				...
				// 4 bytes
				const byte2 = position$1 < end ? src[position$1] : 0;
				if (byte1 > 0xf4 || position$1 >= end || (byte2 & 0xc0) !== 0x80 ||
	...
	if (!nativeAccelerationDisabled) {
		...
		try {
			if (typeof require == 'function')
				...
				extractor = module.createRequire((_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('test.js', document.baseURI).href))('msgpackr-extract');
			if (extractor)
	...
	let allSampleData = [];
	for (let i = 1; i < 6; i++) {
		allSampleData.push(JSON.parse(fs.readFileSync(new URL(`./example${i > 1 ? i : ''}.json`, (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('test.js', document.baseURI).href)))));
	}
	allSampleData.push({
