"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SocketBroadcasterService = void 0;
const common_1 = require("@nestjs/common");
const room_manager_service_1 = require("../../infrastructure/services/room-manager.service");
let SocketBroadcasterService = class SocketBroadcasterService {
    constructor(roomManager) {
        this.roomManager = roomManager;
    }
    emitGatekeeperStatus(userId, status) {
        this.roomManager.broadcastToUser(userId, 'searcher:gatekeeper_status', {
            ...status,
            timestamp: new Date().toISOString(),
        });
    }
    emitMatchOffer(searcherId, offerPayload) {
        this.roomManager.broadcastToUser(searcherId, 'match:offer', {
            ...offerPayload,
            timestamp: new Date().toISOString(),
        });
    }
    emitMatchConfirmed(searcherId, leaverId, matchPayload) {
        const payload = {
            ...matchPayload,
            timestamp: new Date().toISOString(),
        };
        this.roomManager.broadcastToUser(searcherId, 'match:confirmed', payload);
        this.roomManager.broadcastToUser(leaverId, 'match:confirmed', payload);
    }
    emitArrivalPrompt(searcherId, matchId, spotCoords) {
        this.roomManager.broadcastToUser(searcherId, 'prompt:arrival_confirm', {
            matchId,
            spotCoords,
            timestamp: new Date().toISOString(),
        });
    }
    emitFallbackSpot(searcherId, spotPayload) {
        this.roomManager.broadcastToUser(searcherId, 'searcher:routed_db_spot', {
            ...spotPayload,
            timestamp: new Date().toISOString(),
        });
    }
    emitWalletUpdate(userId, balance, formattedBalance) {
        this.roomManager.broadcastToUser(userId, 'wallet:balance_update', {
            balance,
            formattedBalance,
            timestamp: new Date().toISOString(),
        });
    }
};
exports.SocketBroadcasterService = SocketBroadcasterService;
exports.SocketBroadcasterService = SocketBroadcasterService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [room_manager_service_1.RoomManagerService])
], SocketBroadcasterService);
//# sourceMappingURL=socket-broadcaster.service.js.map