...
const vehicle_service_1 = require("../../../src/modules/auth/application/services/vehicle.service");
const in_memory_vehicle_repository_1 = require("../../../src/modules/auth/infrastructure/adapters/in-memory-vehicle.repository");
const exceptions_1 = require("../../../src/common/exceptions");
describe('VehicleService (Module 2 Unit Tests)', () => {
    let vehicleService;
    let vehicleRepo;
    beforeEach(() => {
        vehicleRepo = new in_memory_vehicle_repository_1.InMemoryVehicleRepository();
        vehicleService = new vehicle_service_1.VehicleService(vehicleRepo);
    });
    ...
    it('should reject vehicle creation with invalid plate suffix (non-4 digits)', async () => {
        const userId = 'user-123';
        await expect(vehicleService.addVehicle(userId, {
