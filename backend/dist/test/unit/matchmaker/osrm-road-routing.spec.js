...
describe('OsrmRoadRoutingAdapter', () => {
    ...
    it('should fall back gracefully to Haversine routing when OSRM is unreachable', async () => {
        const adapter = new osrm_road_routing_adapter_1.OsrmRoadRoutingAdapter('http://localhost:59999', 500);
        const candidates = [
    ...
    it('should parse valid OSRM /table response correctly when mock server responds', async () => {
        const adapter = new osrm_road_routing_adapter_1.OsrmRoadRoutingAdapter('http://localhost:5000', 1000);
...
//# sourceMappingURL=osrm-road-routing.spec.js.map